...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const a of accounts as any[]) {
      ...
      const accountHash = accountHashes[String(sa.accountNumber)] ?? null;
      const accountWithPositions = accountHash
        ? await accountDetailForPositions(tokens.access_token, accountHash, a)
        : a;

      const accountSa = accountWithPositions?.securitiesAccount ?? sa;
      const bal = accountSa.currentBalances ?? {};
      ...
      const mapped = mapPositions(accountWithPositions, {});
...
async function accountDetailForPositions(accessToken: string, accountHash: string, fallback: any) {
  try {
    return await getAccount(accessToken, accountHash, true);
  } catch (error) {
    console.error("schwab-oauth account detail positions fetch failed", error);
    return fallback;
  }
}
